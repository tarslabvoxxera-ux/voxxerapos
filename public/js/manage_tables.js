    ...
    var do_action = function(action) {
        return function (url, ids) {
            if (confirm($.fn.bootstrapTable.defaults.formatConfirmAction(action))) {
                // DOUBLE VERIFICATION for delete action
                if (action === 'delete') {
                    if (!confirm('⚠️ SECOND CONFIRMATION REQUIRED ⚠️\n\nAre you ABSOLUTELY SURE you want to delete these items?\n\nThis action cannot be undone!')) {
                        return false;
                    }
                }
                
                var postData = {'ids[]': ids || selected_ids()};
                // Add double_confirm flag for delete action
                if (action === 'delete') {
                    postData['double_confirm'] = 'confirmed';
                }
                
                $.post((url || options.resource) + '/' + action, postData, function (response) {
                    // Check if double confirmation is required
                    if (response.require_double_confirm) {
