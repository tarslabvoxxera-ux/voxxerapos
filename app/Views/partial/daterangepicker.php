...
<?php if (empty($config['date_or_time_format'])) { ?>
    ...
    "startDate": "<?= date($config['dateformat'], mktime(0, 0, 0, date("m"), date("d") - 29, date("Y"))) ?>",
...
<?php } else { ?>
    ...
    "alwaysShowCalendars": true,
    "startDate": "<?= date($config['dateformat'] . ' ' . $config['timeformat'], mktime(0, 0, 0, date("m"), date("d") - 29, date("Y"))) ?>",
