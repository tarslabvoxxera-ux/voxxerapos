...
class Items extends Secure_Controller
{
    ...
    public function getGenerateBarcodes(string $item_ids): void
    {
        ...
        foreach ($result as &$item) {
            // Check if item has no barcode - generate and save one
            if (empty($item['item_number']) && isset($item['item_id'])) {
                // Generate a unique numeric barcode: 100 + 9-digit padded item_id = 12 digits
                $generated_barcode = '100' . str_pad($item['item_id'], 9, '0', STR_PAD_LEFT);
                
                // Save the generated barcode to the database
                $save_item = ['item_number' => $generated_barcode];
                $this->item->save_value($save_item, $item['item_id']);
                
                // Update the item array with the new barcode for display
                $item['item_number'] = $generated_barcode;
                
                log_message('info', "Generated and saved barcode {$generated_barcode} for item ID {$item['item_id']}");
            }
        }
        $data['items'] = $result;
    ...
    public function postDelete(): void
    {
        ...
        $logged_in_person_id = $this->employee->get_logged_in_employee_info()->person_id;
        if ($logged_in_person_id != 1) {
            echo json_encode(['success' => false, 'message' => 'Only admin can delete items.']);
            return;
        }

        // Double verification is handled by JavaScript (manage_tables.js)
            ...
            if ($_FILES['file_path']['error'] !== UPLOAD_ERR_OK) {
                ...
                if (file_exists($_FILES['file_path']['tmp_name'])) {
                    ...
                    foreach ($csv_rows as $key => $row) {
                        ...
                        // Normalize row keys - trim whitespace and convert to uppercase for matching
                        $normalized_row = [];
                        foreach ($row as $col_key => $col_value) {
                            $normalized_key = strtoupper(trim($col_key));
                            $normalized_row[$normalized_key] = is_string($col_value) ? trim($col_value) : $col_value;
                        }
                        
                        // Log the row data for debugging
                        log_message('debug', "Processing row " . ($key + 2) . ": " . implode(', ', array_keys($normalized_row)));
                        
                        // Support both custom format and original OSPOS format
                        // Check for custom format columns (now normalized to uppercase)
                        $is_custom_format = isset($normalized_row['ITEM DESCRIPTION']) || isset($normalized_row['STOCK NO']) ||
                                           isset($normalized_row['BARCODE']) || isset($normalized_row['SUB-DIVISION']) || 
                                           isset($normalized_row['RETAIL PRICE']);
                        
                        log_message('error', "Row " . ($key + 2) . " - Is custom format: " . ($is_custom_format ? 'yes' : 'no') . " - Columns: " . implode(',', array_keys($normalized_row)));
                        
                        if ($is_custom_format) {
                            // Custom CSV format mapping - use normalized row
                            $item_id = 0; // New items only in custom format
                            $is_update = false;
                            
                            // Get values with fallbacks for various column name formats
                            // PRODUCT is the Item Name, ITEM DESCRIPTION is the SKU/code
                            $item_name = $normalized_row['PRODUCT'] ?? '';
                            $item_description_code = $normalized_row['ITEM DESCRIPTION'] ?? $normalized_row['ITEMDESCRIPTION'] ?? $normalized_row['ITEM_DESCRIPTION'] ?? '';
                            $category = $normalized_row['SUB-DIVISION'] ?? $normalized_row['SUBDIVISION'] ?? $normalized_row['SUB_DIVISION'] ?? '';
                            ...
                            $hsn_code = $normalized_row['HSN CODE'] ?? $normalized_row['HSNCODE'] ?? $normalized_row['HSN_CODE'] ?? '';
                            ...
                            $full_description = $item_description_code;
                            ...
                            log_message('debug', "Row " . ($key + 2) . ": name='$item_name', category='$category', price='$unit_price', barcode='$barcode'");
                            
                            // Handle BARCODE/STOCK NO - make unique if duplicate exists
                            if (!empty($barcode)) {
                                $original_barcode = $barcode;
                                $suffix = 1;
                                // If barcode exists, append suffix to make it unique
                                while ($this->item->item_number_exists($barcode)) {
                                    $suffix++;
                                    $barcode = $original_barcode . '-' . $suffix;
                                }
                                $item_data['item_number'] = $barcode;
                                if ($barcode !== $original_barcode) {
                                    log_message('info', "Row " . ($key + 2) . ": Barcode '$original_barcode' already exists, using '$barcode' instead.");
                                }
                            } else {
                                // Generate a unique item number if no barcode provided
                                $item_data['item_number'] = uniqid('ITEM-');
                            }
                            
                            // Handle stock quantity from TRANSQTY
                            if ($transqty !== '') {
                                $normalized_row['TRANSQTY'] = $transqty;
                                $row['TRANSQTY'] = $transqty; // Also set in original row for inventory function
                            }
                            
                            // Use normalized row going forward
                            $row = $normalized_row;
                        } else {
                            // Original OSPOS format
                        ...
                        if (!$is_failed_row) {
                            ...
                            if ($is_failed_row) {
                                log_message('error', "Row " . ($key + 2) . " data_error_check FAILED: name='" . ($item_data['name'] ?? '') . "', category='" . ($item_data['category'] ?? '') . "', unit_price='" . ($item_data['unit_price'] ?? '') . "'");
                            }
                        }
                        ...
                        if (!$is_failed_row && $this->item->save_value($item_data, $item_id)) {
                            ...
                            // Save custom attributes for custom CSV format
                            if ($is_custom_format) {
                                $this->save_custom_attributes($row, $item_data['item_id']);
                            }
                            ...
                            if ($is_failed_row) {
                                log_message('error', "Row $failed_row FAILED validation: " . json_encode($item_data));
    ...
    private function data_error_check(array $row, array $item_data, array $allowed_locations, array $definition_names, array $attribute_data): bool    // TODO: Long function and large number of parameters in the declaration... perhaps refactoring is needed
    {
        ...
        foreach ($check_for_empty as $key => $val) {
            if (empty($val) && $val !== '0' && $val !== 0 && !$is_update) {
        ...
        $check_for_numeric_values = [
            'cost_price'    => $item_data['cost_price'] ?? 0,
            'unit_price'    => $item_data['unit_price'] ?? 0,
        ...
        foreach ($definition_names as $definition_name) {
            $attribute_value = $row["attribute_$definition_name"] ?? null;
            if (!empty($attribute_value)) {
                $definition_type = $attribute_data[$definition_name]['definition_type'] ?? null;
                if ($definition_type === null) continue;
    ...
    private function save_attribute_data(array $row, array $item_data, array $definitions): bool
    {
        foreach ($definitions as $definition) {
            ...
            $attribute_value = $row["attribute_$attribute_name"] ?? null;

            // Create attribute value - skip if column doesn't exist in CSV
    ...
    /**
     * Saves custom attributes from the custom CSV format (BRAND, STYLE, COLOR, SIZE, UOM, GST GROUP)
     *
     * @param array $row The CSV row data
     * @param int $item_id The item ID to link attributes to
     * @return void
     */
    private function save_custom_attributes(array $row, int $item_id): void
    {
        // Map CSV column names to attribute definition IDs
        // These IDs match what we created in ospos_attribute_definitions
        ...
        foreach ($custom_attribute_map as $csv_column => $definition_id) {
            ...
            // Save the attribute value and link it to the item
            $this->attribute->saveAttributeValue($value, $definition_id, $item_id, false, 'TEXT');
        }
    }
    ...
    private function save_inventory_quantities(array $row, array $item_data, array $allowed_locations, int $employee_id): void
    {
        ...
        $is_update = isset($row['Id']) ? (bool)$row['Id'] : false;
        
        // Check for custom format TRANSQTY field
        $has_transqty = isset($row['TRANSQTY']) && ($row['TRANSQTY'] !== '' || $row['TRANSQTY'] === '0');
        ...
        foreach ($allowed_locations as $location_id => $location_name) {
            ...
            // Check for TRANSQTY (custom format) first, then location-specific quantity
            if ($has_transqty) {
                $item_quantity_data['quantity'] = $row['TRANSQTY'];
                $this->item_quantity->save_value($item_quantity_data, $item_data['item_id'], $location_id);

                $csv_data['trans_inventory'] = $row['TRANSQTY'];
                $this->inventory->insert($csv_data, false);
                break; // Only add to first location for custom format
            } elseif (!empty($row["location_$location_name"]) || (isset($row["location_$location_name"]) && $row["location_$location_name"] === '0')) {
    ...
    private function save_tax_data(array $row, array $item_data): void
    {
        ...
        // Check if tax columns exist (original OSPOS format)
        $tax1_percent = $row['Tax 1 Percent'] ?? null;
        $tax1_name = $row['Tax 1 Name'] ?? '';
        $tax2_percent = $row['Tax 2 Percent'] ?? null;
        $tax2_name = $row['Tax 2 Name'] ?? '';

        if (is_numeric($tax1_percent) && $tax1_name !== '') {
            $items_taxes_data[] = ['name' => $tax1_name, 'percent' => $tax1_percent];
        }

        if (is_numeric($tax2_percent) && $tax2_name !== '') {
            $items_taxes_data[] = ['name' => $tax2_name, 'percent' => $tax2_percent];
        }
