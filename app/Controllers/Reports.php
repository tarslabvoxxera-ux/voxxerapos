...
use App\Models\Reports\Comprehensive_sales;
...
class Reports extends Secure_Controller
{
    ...
    private Comprehensive_sales $comprehensive_sales;
    ...
    public function __construct()
    {
        ...
        $this->comprehensive_sales = model(Comprehensive_sales::class);
    ...
    /**
     * Show input form for Comprehensive Sales Report
     */
    public function comprehensive_sales_input(): void
    {
        echo view('reports/comprehensive_sales_input');
    }

    /**
     * Comprehensive Sales Report - Exports everything to Excel
     * Includes: Employee, Customer, Items, Payment Types, All Details
     *
     * @param string $start_date
     * @param string $end_date
     * @param string $sale_type
     ...
     * @return void
     */
    public function comprehensive_sales(string $start_date, string $end_date, string $sale_type = 'all', string $location_id = 'all'): void
    {
        $this->clearCache();
        ...
        $this->comprehensive_sales->create($inputs);
        $report_data = $this->comprehensive_sales->getData($inputs);
        $summary_data = $this->comprehensive_sales->getSummaryData($inputs);

        // Export to Excel/CSV
        helper(['text', 'number']);
        
        $filename = 'Comprehensive_Sales_Report_' . date('Y-m-d_His') . '.csv';
        ...
        // Headers
        $headers = $this->comprehensive_sales->getDataColumns();
        ...
        fputcsv($output, ['Total Transactions', $summary_data['total_transactions']]);
        fputcsv($output, ['Total Items Sold', $summary_data['total_items_sold']]);
        fputcsv($output, ['Total Subtotal', $summary_data['total_subtotal']]);
        fputcsv($output, ['Total Tax', $summary_data['total_tax']]);
        fputcsv($output, ['Total Sales', $summary_data['total_sales']]);
        fputcsv($output, ['Total Cost', $summary_data['total_cost']]);
        fputcsv($output, ['Total Profit', $summary_data['total_profit']]);
    ...
    /**
     * Simple Customer Purchase History - Input form
     * Just search by name or phone
     */
    public function customer_history_input(): void
    {
        $this->clearCache();
        $data = ['config' => $this->config];
        echo view('reports/customer_history_input', $data);
    }

    /**
     * Customer Purchase History Report
     * Shows all purchases by a customer
     */
    public function customer_history(string $customer_identifier): void
    {
        $this->clearCache();
        
        // Decode the customer identifier
        $customer_identifier = urldecode($customer_identifier);
        
        // Try to find customer by the identifier (could be name, phone, or ID)
        $customer_id = null;
        $customer_info = null;
        
        // First try to extract person_id if it's in format "ID Name"
        if (preg_match('/^(\d+)\s/', $customer_identifier, $matches)) {
            ...
            foreach ($customers as $c) {
                ...
                if (stripos($full_name, $customer_identifier) !== false || 
                    stripos($c->phone_number ?? '', $customer_identifier) !== false) {
                    $customer_id = $c->person_id;
                    break;
                }
            }
        }
        ...
        // Get all sales for this customer - use a wide date range
        $db = \Config\Database::connect();
        $builder = $db->table('ospos_sales s');
        $builder->select('
            s.sale_id,
            s.sale_time,
            s.invoice_number,
            ...
            SUM(si.quantity_purchased * si.item_unit_price) as subtotal
        ');
        $builder->join('ospos_people e', 's.employee_id = e.person_id', 'left');
        $builder->join('ospos_sales_items si', 's.sale_id = si.sale_id', 'left');
        $builder->where('s.customer_id', $customer_id);
        $builder->where('s.sale_status', 0); // Completed sales only
        $builder->groupBy('s.sale_id');
        $builder->orderBy('s.sale_time', 'DESC');
        
        $sales = $builder->get()->getResultArray();
        
        // Calculate totals
        $total_purchases = count($sales);
        $total_amount = array_sum(array_column($sales, 'subtotal'));
        ...
        echo view('reports/customer_history', $data);
    }
