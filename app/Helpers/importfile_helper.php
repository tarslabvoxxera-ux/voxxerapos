...
function generate_import_items_csv(array $stock_locations, array $attributes): string
{
    ...
    // Custom format matching user's inventory CSV structure (BARCODE is optional - will use STYLE or ITEM DESCRIPTION if not provided)
    $csv_headers .= '"ITEM DESCRIPTION",PRODUCT,BRAND,STYLE,COLOR,SIZE,"SUB-DIVISION",UOM,"HSN CODE","GST GROUP","COST PRICE","RETAIL PRICE",TRANSQTY';
...
/**
 * Generates CSV template in original OSPOS format
 * @param array $stock_locations
 * @param array $attributes
 * @return string
 */
function generate_import_items_csv_original(array $stock_locations, array $attributes): string
{
    $csv_headers = pack('CCC', 0xef, 0xbb, 0xbf);
